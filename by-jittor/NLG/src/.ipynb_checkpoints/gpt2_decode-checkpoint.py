...
if __name__ == "__main__":
    ...
    with open(args.sample_file, 'r') as sample_reader, \
        ...
        print(len(references),len(hypothesis),'zhr查看写入长度是否对齐？')
        ...
        if args.ref_type == 'e2e':
            with open(args.output_ref_file, 'w', encoding='utf8') as ref_writer:
                for ref, hyp in zip(references, hypothesis):
                    ...
                    ref_writer.write('\n')
                    ### 解决ref与pred文件长度不一致的问题：过滤句内换行符
                    
                    sentence = post_process(hyp, args.tokenize, args.lower)
                    sentence = sentence.replace('\n',' ')
                    
                    pred_writer.write(sentence + '\n')
